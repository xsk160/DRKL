...
def train_mini_batch_drkl(model, feats, labels, target, batch_size, optimizer, lamb=1):
    """
    Train MLP for large datasets. Process the data in mini-batches. The graph is ignored, node features only.
    lamb: weight parameter lambda
    """
    ...
    for i in range(num_batches):
        ...
        logits_out = model(None, feats[idx_batch[i]])
        logits_student = logits_out.log_softmax(dim=1)
        logits_teacher = labels[idx_batch[i]]

        alpha, beta, temperature = 25, 1, 1

        loss = drkl_loss(logits_student, logits_teacher, alpha, beta, temperature)
